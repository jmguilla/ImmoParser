...
public class GoogleGeocodeClient {
     ...
     */
    public final JSONObject getReverseGeocodeJson(BigDecimal lat, BigDecimal lng)
            throws IOException, JSONException {
        JsonBuilder builder = new JsonBuilder();
        synchronized (GEOCODE_LOCL) {
            long tempo = System.currentTimeMillis() - LAST_GEOCODE_CALL
                    - GEOCODE_TEMPO_PACE;
            if (tempo > 0L) {
            ...
            URL url = new URL(BASE_URL + PARAM_JSON + "?" + PARAM_LATLNG + lat
                    + "," + lng + "&" + PARAM_SENSOR);
     ...
     */
    public final Document getReverseGeocodeXML(BigDecimal lat, BigDecimal lng)
            throws IOException {
        ...
        synchronized (GEOCODE_LOCL) {
            ...
            URL url = new URL(BASE_URL + PARAM_XML + "?" + PARAM_LATLNG + lat
                    + "," + lng + "&" + PARAM_SENSOR);
            System.out.println("GEOCODE call to " + url.toExternalForm());
