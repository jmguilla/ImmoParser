...
class DescriptionDAOFile implements DescriptionDAO {
    ...
    private void save(Description toSave,
            final Hashtable<String, Hashtable<Type, OutputStream>> cache,
            String directory) throws IOException {
        Hashtable<Type, OutputStream> tmp = cache.get(toSave.zipCode);
        final String zipCode = (toSave.zipCode != null ? toSave.zipCode
                : toSave.city);
        ...
        if (os == null) {
            File tmpDir = new File(System.getProperty("user.home")
                    + File.separator + ROOT_DIR_NAME + File.separator
                    + directory + File.separator + timeStamp + File.separator
                    + zipCode);
            tmpDir.mkdirs();
        ...
        sb.append(";");
        sb.append(toSave.area);
        sb.append(";");
        sb.append(toSave.creation);
        sb.append(";");
        ...
        for (String zipCode : validCache.keySet()) {
            ...
            for (Type type : tmp.keySet()) {
                try {
                    ...
                    System.err
                            .println("Error while closing valid stream for zip: "
                                    + zipCode + " and type: " + type);
                    e.printStackTrace();
                }
            }
        }
        ...
        for (String zipCode : rejectedCache.keySet()) {
            ...
            for (Type type : tmp.keySet()) {
                try {
                    ...
                    System.err
                            .println("Error while closing rejected stream for zip: "
                                    + zipCode + " and type: " + type);
                    e.printStackTrace();
                }
            }
