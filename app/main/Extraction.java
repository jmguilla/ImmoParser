...
public class Extraction {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            for (WebParser parser : parsers) {
                ...
                parser.extractDescriptions(type, zipCodes, new BigDecimal(
                        VALIDITY_THRESHOLD), result, rejected);
                ...
                System.out.println("******************** Persisting donne in "
                        + (System.currentTimeMillis() - startPersist)
                        + "ms ********************");
                System.out
                        .println("******************** Extraction processed in "
    ...
    private static void persist(Hashtable<Type, ArrayList<Description>> result,
        ...
        for (Type type : result.keySet()) {
            for (Description description : result.get(type)) {
                try {
                    ...
                    // if((previous = Description.find.byId(description.url)) ==
                    // null){
                    // description.save();
                    // }else{
                    ...
                    System.err.println("Exception occurred while saving "
                            + description);
                    e.printStackTrace();
                }
            }
    ...
    private static void printValidStats(
            Hashtable<Type, ArrayList<Description>> result) {

    }

    private static void printRejectStats(
            Hashtable<Type, ArrayList<Description>> result) {
